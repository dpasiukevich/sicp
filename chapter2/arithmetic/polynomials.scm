...
(define (install-dense-term-list-package)
  ...
  (put 'first-term '(dense-term-list) (lambda (term-list) (list (- (length term-list) 1) (car term-list))))
  (put 'rest-terms '(dense-term-list) (lambda (term-list) (tag (cdr term-list))))
           ...
           (let ((term-order (order term))
             ...
             (cond ((< term-order head-order)
                   ...
                   ((= term-order head-order)
                    (tag (cons (add (coeff term)
                                    (car term-list))
                               (cdr term-list))))
                   (else (adjoin-term term (tag (cons 0 term-list)))))))))
  'done)
